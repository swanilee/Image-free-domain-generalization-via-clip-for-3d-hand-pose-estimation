...
from manopth.manopth.tensutils import (th_posemap_axisang, th_with_zeros, th_pack,
    ...
    def forward(self,
        ...
        if self.use_pca or self.joint_rot_mode == 'axisang':
            ...
            th_full_pose = torch.cat([
                ...
                self.th_hands_mean.to(device) + th_full_hand_pose
            ], 1)
        ...
        if th_betas is None or th_betas.numel() == 1:
            ...
            th_v_shaped = torch.matmul(self.th_shapedirs.to(device),
                                       th_betas.transpose(1, 0)).permute(
                                           2, 0, 1) + self.th_v_template.to(device)
            th_j = torch.matmul(self.th_J_regressor.to(device), th_v_shaped).to(device)
            # th_pose_map should have shape 20x135

        th_v_posed = th_v_shaped + torch.matmul(
            self.th_posedirs.to(device), th_pose_map.transpose(0, 1).to(device)).permute(2, 0, 1)
        # Final T pose with transformation done !

        # Global rigid transformation

        root_j = th_j[:, 0, :].contiguous().view(batch_size, 3, 1).to(device)
        ...
        th_T = torch.matmul(th_results2, self.th_weights.transpose(0, 1)).to(device)

        th_rest_shape_h = torch.cat([
            th_v_posed.transpose(2, 1).to(device),
            torch.ones((batch_size, 1, th_v_posed.shape[1]),
                       dtype=th_T.dtype,
                       device=th_T.device),
        ], 1)

        th_verts = (th_T * th_rest_shape_h.unsqueeze(1)).sum(2).transpose(2, 1).to(device)
        th_verts = th_verts[:, :, :3]
