...
import sys
import manopth
sys.path.insert(0, "/data/sglee/colorhandpose3d-pytorch/main/manopth/manopth")
...
class MANO(object):
    ...
    def get_layer(self):
        ...
        # return ManoLayer(mano_root=osp.join('manopth', 'mano', 'models'), flat_hand_mean=False,
        #                  use_pca=False)  # load right hand MANO model

        # True for HO3D False, for FreiHAND False flat_hand_mean


if __name__ == '__main__':
    mano = MANO()
    print(mano.joint_regressor.shape)
    np.savetxt('joint_regressor.txt', mano.joint_regressor)